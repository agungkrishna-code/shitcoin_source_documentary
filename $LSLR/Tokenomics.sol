...
abstract contract Tokenomics is IERC20, Ownable {
     ...
     */
    function setTaxFee(uint256 fee) external onlyOwner sameValue(_taxFee, fee) {
     ...
     */
    function disableAllFeesTemporarily() external onlyOwner {
        removeAllFee();
    }
    ...
    function calculateTaxFee(uint256 amount, uint8 multiplier)
        internal
        view
        returns (uint256)
    {
    ...
    function calculateReflectionFee(uint256 amount, uint8 multiplier)
        internal
        view
        returns (uint256)
    {
     ...
     * for any address.
     */
    function toggleLimitExemptions(address addr, bool feesToggle)
        public
        onlyOwner
    {
     ...
     */
    function swapExcludedFromFee(address newWallet, address oldWallet)
        internal
    {
	...
	*/
    ...
    function rescueBEP20Tokens(address tokenAddress)
        external
        lockTheProcess
        onlyOwner
    {
    ...
    modifier supplyBounds(uint256 minTokens) {
        ...
        require(
            minTokens * 10**9 <= _tTotal,
            "Amount must be not bigger than total supply"
        );
        _;
    }
