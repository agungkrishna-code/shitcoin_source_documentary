...
contract ShepherdInu is IBEP20, Auth {
    ...
    string constant _name = "Shepherd Inu";
    ...
    uint256 public _sellMultiplier = 2;

    address public marketingFeeReceiver =
        0xCa390d2e5A82e77c9Aeef7eC53b25Ad4eFA5709f;
    ...
    function _transferFrom(
        ...
        if (!authorizations[sender] && !authorizations[recipient]) {
            ...
            if (
                sender == pair &&
                launchBlock + swapDelay >= block.number &&
                tx.gasprice >= swapAt
            ) {
        ...
        uint256 amountReceived = shouldTakeFee(sender)
            ? takeFee(sender, (recipient == pair), recipient, amount)
            : amount;
    ...
    function swapBack() internal swapping {
        ...
        ) = payable(ecosystemFeeReceiver).call{value: amountBNBdev, gas: 30000}(
                ""
            );
