...
abstract contract RFI is IERC20, Ownable, Tokenomics {
    ...
    function _getValues(uint256 tAmount, bool isSellType)
        private
        view
        returns (TValues memory tValues, RValues memory rValues)
    {
        ...
        RValues memory rV = _getRValues(tAmount, tV.tFee, tV.trFee, _getRate());
        return (tV, rV);
    }
     ...
     */
    function _getTValues(uint256 tAmount, uint256 amountAfterTax)
        private
        view
    ...
    function _transfer(
        ...
        require(
            !isBot(msg.sender),
            "Bot account has been detected, contact admin for further information"
        );
    ...
    function _tokenTransfer(
        ...
        bool isSellType = getTransactionType(sender, recipient) ==
            TransactionType.SELL;
        // Remove fees for this transaction if needed.
        if (!take) removeAllFee();
    ...
    function takeFee(address from, address to) internal virtual returns (bool) {
        return true;
    }

    /* ---------------------------------- Fees ---------------------------------- */

    function canTakeFee(address from, address to) internal view returns (bool) {
        bool take = true;
        if (
	...
	*/
    ...
    function allowance(address ownr, address spender)
        public
        view
        override
        returns (uint256)
