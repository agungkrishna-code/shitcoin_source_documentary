...
contract NoCapCoin is Context, IBEP20, Ownable {
    ...
    string private _name = "NoCapCoin";
    string private _symbol = "NCC";
    ...
    uint256 public _antiWhaleThreshold = 4500000000000 * 10**9;
        ...
        */
        if (_isAntiWhaleEnabled && !_isExcludedFromAntiWhale[to]) {
            if (from == uniswapV2Pair || from == address(uniswapV2Router)) {
                require(
                    balanceOf(to) <= _antiWhaleThreshold,
